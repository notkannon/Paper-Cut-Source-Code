local server = shared.Server
local client = shared.Client
local IS_CLIENT = client ~= nil

-- declarations
local ReplicatedStorage = game:GetService("ReplicatedStorage")

-- getting refx module link
local Refx = require(ReplicatedStorage.Package.refx)
local Flashlight = Refx.CreateEffect('ItemFlashlightEnable')

-- methods
function Flashlight:OnStart()
	local instance: Tool = self.Configuration[1]
	instance.Base.Neon.Material = Enum.Material.Neon
	instance.Base.Spotlight.Enabled = true
	instance.Base.Beam.Enabled = true
	instance.Base.On:Play()
end

return Flashlight