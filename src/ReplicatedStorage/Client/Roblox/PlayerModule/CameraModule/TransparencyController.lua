...
function TransparencyController:IsValidPartToModify(part: Instance)
	if (part.Parent and part.Parent:IsA("Accessory") and hiddenAccessories[part.Parent.AccessoryType]) or part.Name == "Head" then
...
function TransparencyController:Update(dt)
	...
	--		for child, _ in pairs(self.cachedParts) do
	--			if not (child or child.Parent) then
	--				continue
	--			end
				
	--			-- keep the arms visible
	--			if (child.Parent and child.Parent:IsA("Accessory") and hiddenAccessories[child.Parent.AccessoryType]) or child.Name == "Head" then
	--				pcall(function()
