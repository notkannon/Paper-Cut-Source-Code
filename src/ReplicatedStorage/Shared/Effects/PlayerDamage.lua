...
local PlayerDamage = Refx.CreateEffect("PlayerDamage") :: Impl
...
local function CreateBlood(character: Model)
	...
	NewTrail:ApplyImpulse(reverseDirection * .02)
	...
	Hitbox.OnHit:Connect(function(hit: BasePart, _, raycastResult: RaycastResult?)
		...
		for _, Particle in ipairs(ReplicatedStorage.Assets.Particles.Blood.Puddle:GetChildren()) do
			Particle:Clone().Parent = NewBlood
		end

		EffectsUtility.EmitDescendants(NewBlood, true)

		Debris:AddItem(NewTrail, 3)
		Debris:AddItem(NewBlood, 5)
...
function PlayerDamage.OnStart(self: Effect, character: Instance, amount: number)
	...
	-- THINK TWICE BEFORE READDING. THIS HIGHLIGHT OVERRIDES EXISTING HIGHLIGHTS AND RUINS MANY PASSIVES (Oliver's, Circle's, Engel's etc)
	local Highlight = Instance.new("Highlight")
	Highlight.Parent = character

	local Alpha = amount / Humanoid.MaxHealth

	local Sound = SoundUtility.CreateTemporarySound(
		SoundUtility.GetRandomSoundFromDirectory(
			SoundUtility.Sounds.Players.Gore.Dismemberment.Obliterate
		)
	)

	Sound.Parent = character.HumanoidRootPart
	Sound.Volume = math.clamp(Alpha^2 + 0.2, 0, 1)

	Utility.ApplyParams(Highlight, {
		DepthMode = Enum.HighlightDepthMode.Occluded,
		FillColor = Color3.fromRGB(103, 0, 0),

		OutlineTransparency = 1,
		FillTransparency = 0.35
	})

	for i = 1, math.clamp(math.ceil(amount / 10), 2, 10) do
		CreateBlood(character)
	end

	TweenUtility.PlayTween(Highlight, TweenInfo.new(0.35), {
		FillTransparency = 1
