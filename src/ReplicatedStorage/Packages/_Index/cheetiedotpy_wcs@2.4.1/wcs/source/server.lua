...
local toSerializeablePayload = TS.import(script, script.Parent.Parent, "include", "node_modules", "@rbxts", "charm-payload-converter", "out").toSerializeablePayload
...
do
	...
	function Server:constructor()
		...
		self._internal_syncer = sync.server({
			atoms = {
	...
	function Server:Start()
		...
		self._internal_syncer:connect(function(player, ...)
			...
			-- ▼ ReadonlyArray.map ▼
			local _newValue = table.create(#modified)
			for _k, _v in modified do
				_newValue[_k] = toSerializeablePayload(_v, _k - 1, modified)
			end
			-- ▲ ReadonlyArray.map ▲
			local serializeable = _newValue
			local serialized = dispatchSerializer.serialize(serializeable)
