...
local _charm = TS.import(script, script.Parent.Parent, "include", "node_modules", "@rbxts", "charm", "src")
local subscribe = _charm.subscribe
local sync = _charm.sync
local fromSerializeablePayload = TS.import(script, script.Parent.Parent, "include", "node_modules", "@rbxts", "charm-payload-converter", "out").fromSerializeablePayload
...
local dispatchSerializer = _serdes.dispatchSerializer
...
do
	...
	function Client:constructor()
		...
		self._internal_clientSyncer = sync.client({
			atoms = {
				atom = clientAtom,
			},
		})
	...
	function Client:Start()
		...
		ClientEvents.sync:connect(function(serialized)
			local payloads = dispatchSerializer.deserialize(serialized.buffer, serialized.blobs)
			local __internal_clientSyncer = self._internal_clientSyncer
			-- ▼ ReadonlyArray.map ▼
			local _newValue = table.create(#payloads)
			for _k, _v in payloads do
				_newValue[_k] = fromSerializeablePayload(_v, _k - 1, payloads)
			end
			-- ▲ ReadonlyArray.map ▲
			__internal_clientSyncer:sync(unpack(_newValue))
		end)
		ClientEvents.start()
